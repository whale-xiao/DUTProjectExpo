...
**当前状态（2026-09-14）**：**已进入编码期**。`backend/`（Go 后端，含前台接口 + `/api/admin/*` 后台接口）、`frontend-showcase/`（前台展示站）、`frontend-admin/`（Vue3 + Element Plus 管理后台）均已实现并可运行。运行说明见 `docs/06-环境配置.md`。

这是一个 9 周课程项目（9 周计划表见根目录 `*.xlsx`；`docs/开发流程.md` 是阶段里程碑摘要，标记"勿改"）。评审节点：9/8 需求分析、9/11 原型、9/24 数据库+概要设计、10/16 代码终版、10/30 答辩。

## 协作角色（重要）

- **代码由团队成员维护**：后端 肖文浩 / 王泽远 / 郭晨浩；前端 马龙 / 郑亚林；组长（终审/提交）**肖文浩**。
- Claude 默认是**顾问 + 文档维护者**：除非用户明确要求"帮我改/写代码"，否则不在 `backend/`、`frontend-showcase/`、`frontend-admin/` 直接修改业务代码；多做方案、答疑、报错定位、设计文档维护。
- 运行服务由用户在自己终端掌控；Claude 验证时使用备用端口并事后清理，不留驻占用 8080/5173/5174。

## 团队与分工（2026-09-06）

| 角色 | 成员 | 范围 |
| --- | --- | --- |
| 组长 / 责任者 | 肖文浩 | 终审、提交、需求/数据库评审材料 |
| 后端 | 肖文浩 · 王泽远 · 郭晨浩 | Go 接口、数据库、内容块事务、部署运维 |
| 前端 | 马龙 · 郑亚林 | 前台展示站 + 后台管理端（页面/组件/联调） |
...
## docs 状态（2026-09-14 核对）

`docs/` 已收敛为**单套现行文档**，早期旧版（5 表/BLOB/`:slug` 那一代）已全部移除，目录里不再有"已被取代"的标注文件。

- **现行设计/运行基准**：`01-需求分析.md`（需求，9/8 已评审）、`02-后台UI设计.md`、`03-数据库设计.md`（9 表基准）、`04-UML设计.md`（接口已同步实现）、`06-环境配置.md`（V2.0 实测）、`09-前台设计方案.md`、`10-调用链设计.md`、`11-UI设计和性能优化.md`（§11 编号与文件名中的 `11-` 一致，此前引用为 `UI设计和性能优化.md` 已过时）。
- **待评审**：`12-前台影院化改造方案.md`（首页影院头图 + 详情页电影化，**仅前端**、后端零改动；取代 `09` §5.1/§5.4，编码前需组长与前端负责人确认）。
- **索引**：`00-README.md` 为最新索引。
- **设计稿截图**：`登录界面.jpg`、`后台展示.jpg`、`前台展示.jpg`、`全部项目.png`、`项目详细.jpg`。
- **正文中出现的文档内链接可能按 §编号互相引用**（如 `06` 指 `06-环境配置.md`），改文件名时需同步。

> ⚠️ 待确认：CLAUDE.md 旧版曾提到 `docs/需求变更记录.md`、`docs/界面功能点与分工矩阵.md` 两份课程交付物，以及根目录 `若依参考架构.md`、`第一版开发要求.txt`、`开发流程.md`——**当前工作区中这些文件均不存在**。若它们在其他分支/成员机器上，请在需要时补回。
